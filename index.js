const discussCard = async (searching = "") => {
  ...
  data.posts.forEach((items) => {
    ...
    <img class="rounded-md" src="${items.image}" alt="">
      <div id = "active"
        class = "bg-emerald-500 size-3  rounded-full absolute -top-1 -right-1"
...
const latestPost = async () => {
  ...
  data.forEach((items) => {
    ...
    newDiv.innerHTML = `<img class ="rounded-2xl" src="${
      items.cover_image
    }" alt="" />
...
const counterElement = document.getElementById("countNumber");
let countNumber = 0;
...
const emailHandlar = (data) => {
  ...
  countNumber++;
  counterElement.textContent = `(${countNumber})`;
